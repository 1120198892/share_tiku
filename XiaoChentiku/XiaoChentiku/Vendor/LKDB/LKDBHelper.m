...
@property (unsafe_unretained,nonatomic) FMDatabase *db;
...
- (instancetype)init
{
    return [self initWithDBName:@"xiu"];
}
...
- (void)setDBPath:(NSString *)filePath
{
    ...
    [self.dbQueue close];
    self.dbQueue = [[FMDatabaseQueue alloc] initWithPath:filePath];
...
- (void)syncBlock:(void(^)(void))block
{
...
- (void)insertArryToDB:(NSMutableArray *)models callback:(void(^)(BOOL result))block
{
    [self asyncBlock:^{
        [self executeDB:^(FMDatabase *db) {
            [db beginTransaction];
            for (NSObject *model in models) {
               [self insertBase:model];
            }
            [db commit];
            block(1);
        }];
...
-(BOOL)insertBase:(NSObject*)model{
    ...
    NSString* insertSQL = [NSString stringWithFormat:@"insert into %@(%@) values(%@)",[modelClass getTableName],insertKey,insertValuesString];
...
- (void)replaceArryToDB:(NSMutableArray *)models callback:(void(^)(BOOL result))block
{
    [self asyncBlock:^{
        [self executeDB:^(FMDatabase *db) {
            [db beginTransaction];
            for (NSObject *model in models) {
                [self replaceBase:model];
            }
            [db commit];
            block(1);
        }];
...
-(BOOL)replaceBase:(NSObject*)model{
